...
public class UseCarAdapter extends RecyclerView.Adapter {
    ...
    public int getItemViewType(int position) {
        if (useCarData.getResult().getNewslist().get(position).getMediatype() == 10) {
            return PICTURE;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        switch (viewType) {
            case MESSAGE:
                ...
                messageViewHolder.titleTv.setText(useCarData.getResult().getNewslist().get(position).getTitle());
                messageViewHolder.countTv.setText(useCarData.getResult().getNewslist().get(position).getReplycount() + "评论");
                messageViewHolder.timeTv.setText(useCarData.getResult().getNewslist().get(position).getTime());
            ...
            case PICTURE:
                ...
                pictureViewHolder.pTitleTv.setText(useCarData.getResult().getNewslist().get(position).getTitle());
                pictureViewHolder.pTimeTv.setText(useCarData.getResult().getNewslist().get(position).getTime());
                pictureViewHolder.pCountTv.setText("图说  " + useCarData.getResult().getNewslist().get(position).getReplycount() + "评论");

                String url = useCarData.getResult().getNewslist().get(position).getIndexdetail();
                String[] urls = null;
                urls = url.split("㊣");
